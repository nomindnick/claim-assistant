...
def search_documents(query: str, top_k: Optional[int] = None) -> Tuple[List[Dict[str, Any]], List[float]]:
...
def answer_question(question: str, chunks: List[Dict[str, Any]]) -> str:
    ...
    for i, chunk in enumerate(chunks):
        formatted_chunks += f"DOCUMENT {i+1}: {chunk['file_name']} (Page {chunk['page_num']})\n"
    ...
    if debug_mode:
        console.log("DEBUG MODE: Generating fake answer")
        # Generate a simple answer based on the chunks
        docs = [f"{chunk['file_name']} (page {chunk['page_num']})" for chunk in chunks[:2]]
        docs_str = ", ".join(docs)
        
        # Extract first sentences from each chunk to simulate an answer
        snippets = []
        ...
        answer = f"""Based on the documents I found, the answer to your question "{question}" can be found in {docs_str}.
        
{snippets_str}

You may want to examine these documents in more detail for complete information."""
        return answer
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a construction claim assistant skilled at answering questions based on provided document excerpts."},
...
def display_results(
    ...
    if markdown_output:
        ...
        for i, (chunk, score) in enumerate(zip(chunks, scores)):
            md += f"## Source {i+1}: {chunk['file_name']} (Page {chunk['page_num']})\n\n"
...
def query_documents(
    ...
    if not chunks:
        console.print("[bold red]No relevant documents found. Have you ingested any PDFs?")
        return
