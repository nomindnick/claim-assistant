    ...
    for question, expected in fixture_data.items():
        ...
        for chunk in chunks:
            ...
            if len(expected_parts) == 2:
                ...
                if (expected_file in chunk["file_name"] and 
                    int(expected_page) == chunk["page_num"]):
                    found = True
                    break
            else:
...
def test_hallucination():
    ...
    assert hallucination_rate <= 0.15, f"Hallucination rate too high: {hallucination_rate:.2f}"
